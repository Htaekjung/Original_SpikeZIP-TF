...
def main(args):
    ...
    if args.mode.count("QANN") > 0:
        ...
        if args.rank == 0:
            ...
            # [수정된 로직] SNN 뉴런 레이어 탐색 포함
            from spike_quan_layer import IFNeuron # 클래스 타입 체크용
            for name, module in model.named_modules():
                # (1) 일반 연산 레이어 저장
                if isinstance(module, (nn.Linear, nn.Conv2d)): 
                    hook_instance = SaveOutputNpyHook(name, save_dir, max_batches=1)
                ...
                # (2) SNN 뉴런 레이어(IFNeuron) 저장 - 이름에 '_post' 추가
    ...
    model.to(device)
    # ====================================================
    # [수정됨] SNN 스파이크 발생률 측정 및 "리셋" 추가
    # ====================================================
    if args.mode == "SNN" and args.rank == 0:
        print("\n[Analysis] Measuring Spike Firing Rates (Accumulated)...")
        from spike_quan_layer import IFNeuron

        # 1. 훅 인스턴스 생성 및 등록
        # (나중에 compute_rate를 호출해야 하므로 인스턴스를 리스트에 저장해둬야 함)
        hook_instances = [] 
        handle_list = []
        ...
        for name, module in model.named_modules():
            if isinstance(module, IFNeuron):
                ...
                hook_instances.append(hook)       # 훅 인스턴스 보관
                handle_list.append(handle)        # 핸들 보관 (제거용)
        ...
        with torch.no_grad():
            for images, target in data_loader_val:
                ...
                model(images) # Forward Pass (여기서 T번 반복하며 hook이 누적됨)
                break         # 1번만 실행
        
        # 3. 누적된 결과 계산 및 딕셔너리 저장 [핵심 변경점]
        spike_rates = {} # 초기화
        ...
        # 5. [중요] IFNeuron 상태 리셋 (정확도 하락 방지)
        print("[Info] Resetting IFNeuron states (q, acc_q)...")
        for name, module in model.named_modules():
            if isinstance(module, IFNeuron):
                module.reset()
        ...
        if spike_rates:
            ...
            plt.ylabel("Firing Rate (Avg over Time)", fontsize=12)
            ...
            # 터미널 출력
            print(f"\n{'Layer Name':<50} | {'Rate':<10} | {'Status'}")
            print("-" * 80)
            for name, rate in spike_rates.items():
                if rate == 0.0:
                    print(f"{name:<50} | {rate:<10.4f} | 💀 Dead")
                elif rate > 0.8:
                    print(f"{name:<50} | {rate:<10.4f} | 🔥 Bursting")
            print("-" * 80 + "\n")
    # ====================================================
