...
def init_distributed_mode(args):
    ...
    torch.distributed.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                         world_size=args.world_size, rank=args.rank, timeout=datetime.timedelta(seconds=10800))
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, is_best=False):
    ...
    if is_best:
        ...
        file_name = 'checkpoint-%s.pth' % str(epoch)
    ...
    if loss_scaler is not None:
        ...
        # DeepSpeed 등 외부 엔진 사용 시 태그 설정
        tag = "checkpoint-best" if is_best else "checkpoint-%s" % str(epoch)
