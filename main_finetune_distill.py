...
from spike_quan_wrapper import myquan_replace, SNNWrapper, remove_softmax
...
def main(args):
    ...
    checkpoint_teacher = torch.load(args.pretrain_teacher, map_location='cpu', weights_only=False)
    ...
    if args.distributed:
            ...
            # 2. Rank 0에서만 파라미터 이름 확인 (로그 정리용)
            # torch.distributed.get_rank() == 0 조건을 추가하세요.
            if torch.distributed.get_rank() == 0:
                print("\n=== 파라미터 인덱스 확인 (DDP Error Debug) ===")
                # 에러 로그에 떴던 번호들 (예: 4, 10, 11, 24 등 다양하게 넣어보세요)
                target_indices = [4, 10, 11, 24, 100] 
                
                for i, (name, param) in enumerate(model.module.named_parameters()):
                    if i in target_indices: 
                        print(f"인덱스 {i}: {name} | 학습가능여부(Requires_Grad): {param.requires_grad}")
                print("==============================================\n")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 2. 정확도 체크 및 Best 모델 저장 로직
        acc1 = test_stats['acc1']
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        
        if acc1 > max_accuracy:
            max_accuracy = acc1
            print(f'*** New Best Accuracy: {max_accuracy:.2f}%! Saving checkpoint-best.pth ***')
...
if __name__ == '__main__':
    ...
    if args.output_dir:
        ...
        args.output_dir = os.path.join(args.output_dir,
                                       "{}_{}_{}_{}_{}_act{}_weightbit{}_MyQuan_LSQ+_V2".format(args.project_name, args.model, args.dataset, args.act_layer, args.mode, args.level,args.weight_quantization_bit))
        Path(args.output_dir).mkdir(parents=True, exist_ok=True)
