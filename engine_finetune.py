...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss /= accum_iter
        loss_scaler(loss.float(), optimizer, clip_grad=max_norm,
...
def train_one_epoch_distill_prune(model: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_all /= accum_iter
        loss_scaler(loss_all.float(), optimizer, clip_grad=max_norm,
                    parameters=model.parameters(), create_graph=False,
                    update_grad=(data_iter_step + 1) % accum_iter == 0)
        if (data_iter_step + 1) % accum_iter == 0:
            optimizer.zero_grad()
...
import util.misc as misc
import util.lr_sched as lr_sched # lr_sched가 import 되어 있는지 확인하세요
...
def train_one_epoch_distill(model: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    mixup_fn: Optional[Mixup] = None, log_writer=None,
                    args=None):
    # AMP(혼합 정밀도)를 끄고 FP32로 학습하는 안정적인 버전입니다.
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # -------------------------------------------------------------------------
        # [핵심 수정 1] autocast 제거 (FP32 강제)
        # -------------------------------------------------------------------------
        # with torch.cuda.amp.autocast():  <-- 이 줄을 없앱니다.
        ...
        # get_logits_loss 함수 호출 (import 확인 필요)
        loss_distill = get_logits_loss(outputs_teacher, outputs, targets, args.temp)
        ...
        # -------------------------------------------------------------------------
        # [핵심 수정 2] loss_scaler 대신 일반적인 Backward 사용
        # -------------------------------------------------------------------------
        # loss_scaler(loss_all, ...)  <-- 이 부분 대신 아래 코드를 사용합니다.
