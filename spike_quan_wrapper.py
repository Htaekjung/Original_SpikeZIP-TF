...
class SNNWrapper(nn.Module):
    ...
    def _replace_weight(self,model):
        ...
        for name, child in children:
            ...
            # elif isinstance(child, MyQuan):
            #     # 1. Beta(Offset) 값 추출
            #     # MyQuan에 use_beta가 있고 True이면 beta를 가져오고, 아니면 0.0
            #     if hasattr(child, 'use_beta') and child.use_beta:
            #         beta_val = child.beta.data
            #     else:
            #         beta_val = torch.tensor(0.0)
            #     neurons = IFNeuron(
            #         q_threshold = child.s.data, 
            #         level = child.level,        
            #         beta = beta_val,            
            #         sym = child.sym
            #     )
            ...
            #     # [중요] 반드시 torch.tensor()로 감싸서 전달
            #     # MyQuan의 pos_max가 'full'일 경우를 대비해 float 변환 시 예외 처리
            #     if isinstance(child.pos_max, (int, float)):
            #         neurons.pos_max = torch.tensor(float(child.pos_max)).to(child.s.device)
            #     else:
            #         neurons.pos_max = child.pos_max # 'full'인 경우 그대로 전달

            #     if isinstance(child.neg_min, (int, float)):
            #         neurons.neg_min = torch.tensor(float(child.neg_min)).to(child.s.device)
            #     else:
            #         neurons.neg_min = child.neg_min
    ...
    def _myquan_replace(model,level):
        ...
        for name, child in children:
            ...
            if isinstance(child, Block):
                ...
                # model._modules[name].mlp.act = nn.Sequential(child.mlp.act, MyQuan(level, sym='gelu'))
